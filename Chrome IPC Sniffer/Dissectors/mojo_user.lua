...
local trace_nonce                      = ProtoField.uint32 ("mojouser.tracenonce"  , "Trace Nonce"     , base.HEX)
...
local request_in_frame_             = ProtoField.framenum("mojouser.request_in", "Request In Frame", base.NONE, frametype.REQUEST)
local response_in_frame_            = ProtoField.framenum("mojouser.response_in", "Response In Frame", base.NONE, frametype.RESPONSE)
...
mojo_protocol.fields = {
    ...
    num_bytes, version,
    interface_id, name, flags, trace_nonce, request_id, payload_pointer, payload_interface_ids_pointer,  creation_timeticks_us, request_in_frame_, response_in_frame_,
...
local _request_id = Field.new("mojouser.requestid")
local _trace_nonce = Field.new("mojouser.tracenonce")
...
function mojo_protocol.dissector(buffer, pinfo, tree)
    ...
    structHeaderTree:add_le(trace_nonce,                            buffer(offset,4));                                         offset = offset + 4
    if _version()() >= 1 then
        structHeaderTree:add_le(request_id,                      buffer(offset,8));                                         offset = offset + 8

        add_request_response_fields(pinfo, structHeaderTree)

    end
    if _version()() >= 2 then
        local payload_pointer_value = buffer(offset, 8):le_int64():tonumber()
        structHeaderTree:add_le(payload_pointer,                 buffer(offset,8)):set_text("Payload (pointer): +" .. payload_pointer_value);                       offset = offset + 8
        local interface_ids_pointer_value = buffer(offset, 8):le_int64():tonumber()
        structHeaderTree:add_le(payload_interface_ids_pointer,   buffer(offset,8)):set_text("Payload Interface IDs (pointer): +" .. interface_ids_pointer_value);   offset = offset + 8
    end
    if _version()() >= 3 then
...
function add_request_response_fields(pinfo, header_tree)
	local msg_trace_id = tostring(concat_u32_to_u64(_method_name()(), _trace_nonce()()))
    if msg_trace_ids_to_info[msg_trace_id] == nil then
        -- initialize the info about the message if needed
        msg_trace_ids_to_info[msg_trace_id] = {}
        msg_trace_ids_to_info[msg_trace_id].req_ids = {}
        msg_trace_ids_to_info[msg_trace_id].req_ids[tostring(_request_id()())] = {}
    end

    local req_res_info = msg_trace_ids_to_info[msg_trace_id].req_ids[tostring(_request_id()())]

    if _expects_response()() and not _is_response()() then
        -- save the request, get the response
        req_res_info.req_frame_num = pinfo.number

        local res_frame_number = req_res_info.res_frame_num
        if res_frame_number ~= nil then
            header_tree:add_le(response_in_frame_, res_frame_number):set_generated(true); 
        end
    elseif _is_response()() then 
        -- save the response, get the request
        req_res_info.res_frame_num = pinfo.number

        local req_frame_number = req_res_info.req_frame_num
        if req_frame_number ~= nil then
            header_tree:add_le(request_in_frame_, req_frame_number):set_generated(true); 
        end
    end
end
...
function concat_u32_to_u64(hi, lo)
    local hi64 = UInt64(hi):lshift(32)
    local lo64 = UInt64(lo)
    return hi64 + lo64
end
